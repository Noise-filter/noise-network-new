...
class SocketAddressFactory {
  public:
    ...
    static auto Create(unsigned short family) -> std::unique_ptr<SocketAddressInterface>;
    static auto Create(const std::string& ip, unsigned short port = 0)
    ...
    static auto CreateFromSocket(SOCKET socket) -> std::unique_ptr<SocketAddressInterface>;
};
